...
def test_entity_form_uses_icon_and_template_selectors():
    ...
    icon_marker = next(
        ...
        if isinstance(marker, vol.Marker) and marker.schema == CONF_ICON
    )
    ...
    assert icon_marker.description["suggested_value"] == ""
...
def test_multiple_climate_sources_keep_domain_and_generate_type_aware_helpers(hass):
    ...
    hass.states.async_set(
        ...
        {
            **attributes,
            # SiHAS BCM: fan_only is hot-water-only, not room ventilation.
    ...
    defaults = _reference_entity_defaults(
        hass,
        ["climate.boiler", "switch.hot_water", "climate.air_conditioner"],
    )
    ...
    assert "['heat']" in templates["hvac_modes"]
    assert "climate.air_conditioner" in templates["current_temperature"]
    assert "air_conditioner" in templates["hvac_mode"]
    ...
    hass.states.async_set("climate.boiler", "unavailable")
    assert availability.async_render(parse_result=True) is True
    hass.states.async_set("climate.boiler", "fan_only")
    hass.states.async_set("climate.air_conditioner", "unavailable")
    assert availability.async_render(parse_result=True) is True
    hass.states.async_set("switch.hot_water", "unavailable")
    assert availability.async_render(parse_result=True) is False


def test_combined_climate_helper_refresh_preserves_custom_jinja_per_field(hass):
    """Automatic edits refresh generated BCM/Nest helpers field by field."""
    ...
    generated = _reference_entity_defaults(hass, source_ids)
    current = dict(generated)
    current[CONF_NATIVE_VALUE_TEMPLATES] = {
    ...
    hass.states.async_set(
        ...
        "dry",
        {
            "hvac_modes": ["off", "cool", "dry", "heat_cool"],
            "min_temp": 10,
            "max_temp": 32,
            "target_temp_low": 19,
            "target_temp_high": 25,
    ...
    assert _yaml_value(refreshed[CONF_COMMAND_ACTIONS_JSON]) == _yaml_value(
        refreshed_reference[CONF_COMMAND_ACTIONS_JSON]
    )
...
def test_combined_climate_helper_covers_source_order_and_state_matrix(hass):
    """Exercise every helper branch for BCM hot-water plus room HVAC inputs."""
    boiler_entity_id = "climate.boiler"
    ...
    for ordered_source_ids in permutations(source_ids):
        defaults = _reference_entity_defaults(hass, list(ordered_source_ids))
    ...
    for boiler_state in ("heat", "fan_only", "off", "unknown", "unavailable"):
        for air_conditioner_state in (
            ...
            hass.states.async_set(boiler_entity_id, boiler_state, boiler_attributes)
            hass.states.async_set(
            ...
            assert availability_template.async_render(parse_result=True) is (
                boiler_state not in {"unknown", "unavailable"}
                or air_conditioner_state not in {"unknown", "unavailable"}
            )
...
def test_edit_defaults_repair_legacy_enum_repr_for_every_entity_domain(platform):
    ...
    event_hook = _yaml_value(defaults[CONF_EVENT_HOOKS_JSON])[0]
    ...
    command = _yaml_value(defaults[CONF_COMMAND_ACTIONS_JSON])["legacy_command"]
...
def test_reference_fan_promotes_native_speed_preset_and_motion_options(hass):
    ...
    assert _yaml_value(defaults[CONF_ATTRIBUTES_JSON]) == {"vendor": "kept"}
    command_actions = _yaml_value(defaults[CONF_COMMAND_ACTIONS_JSON])
    assert set(command_actions) == {
        "oscillate",
        "set_direction",
...
def test_reference_entity_defaults_combines_boolean_sources_with_and_template(hass):
    ...
    assert _yaml_value(defaults[CONF_TEMPLATE_SOURCES_JSON]) == {
        ...
        "alarm_ready": "switch.alarm_ready",
    }
...
def test_presence_motion_helper_uses_majority_and_delayed_all_off_clear(
    ...
    assert _yaml_value(single_source_defaults[CONF_DOMAIN_OPTIONS_JSON]) == {
        CONF_CLASS: "motion",
    }
...
def test_number_helper_filters_positive_spikes(hass, states, expected):
    ...
    variables = {
        ...
        for name, state in zip(
            _yaml_value(defaults[CONF_TEMPLATE_SOURCES_JSON]), states, strict=True
        )
    }
...
def test_multiple_geolocation_sources_use_device_tracker_location_helper(hass):
    ...
    assert defaults[CONF_PLATFORM] == "device_tracker"
    assert defaults[CONF_VALUE_TEMPLATE] == ""
    assert CONF_LOCATION_HELPER in _yaml_value(defaults[CONF_DOMAIN_OPTIONS_JSON])
...
def test_auto_helper_refreshes_attribute_jinja_per_key_and_removes_stale_helpers():
    ...
    assert _yaml_value(refreshed[CONF_ATTRIBUTE_TEMPLATES_JSON]) == {
        "added": "{{ state_attr('sensor.new', 'added') }}",
        "customized": "{{ 42 }}",
        "generated": "{{ state_attr('sensor.new', 'generated') }}",
    }
    ...
    assert _yaml_value(forced[CONF_ATTRIBUTE_TEMPLATES_JSON]) == _yaml_value(
...
def test_entity_form_defaults_round_trips_stored_entity_config():
    ...
    assert _yaml_value(defaults[CONF_EVENT_HOOKS_JSON]) == [
        {
            "trigger": "event",
            "event_type": "virtual_layer_manual_update",
            CONF_VALUE_TEMPLATE: "{{ trigger.data.value }}",
...
def test_generated_helpers_are_editable_values_and_yaml_suggestions(hass):
    fan_entity_id = "fan.air_purifier"
    number_entity_id = "number.air_purifier_favorite_speed"
    hass.states.async_set(
        fan_entity_id,
        "on",
        {
    ...
    outer = {marker.schema: (marker, value) for marker, value in schema.schema.items()}
    advanced = outer[CONF_ADVANCED_SETTINGS][1]
    advanced_fields = {
        marker.schema: (marker, value)
        for marker, value in advanced.schema.schema.items()
    }
...
set_preset_mode:
  - choose:
      - conditions: "{{ preset_mode == 'sleep' }}"
        sequence:
          - action: input_select.select_option
            target:
              entity_id: input_select.real_fan_mode
            data:
              option: quiet
"""
    parsed_yaml = _parse_command_actions(yaml_actions, "fan")
...
async def test_matter_fan_helper_delivers_power_preset_and_selected_speeds(hass):
    """Exercise generated actions through VirtualFan, not only Jinja rendering."""
    source = "fan.stepped"
    hass.states.async_set(
        source,
        "on",
        {"percentage": 40, "percentage_step": 20, "preset_modes": ["eco"]},
    )
    generated = _apply_matter_fan_level_helper(
        {
            CONF_PLATFORM: "fan",
