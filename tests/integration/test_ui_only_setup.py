...
async def _choose_add_template_helper(
    ...
    *,
    ...
    if result["step_id"] == "entity_type":
        defaults = suggested_form_values(result["data_schema"])
        result = await hass.config_entries.options.async_configure(
            result["flow_id"],
            {
...
async def test_climate_source_can_be_converted_to_temperature_sensor_in_add_flow(hass):
    """The extra conversion step must generate a typed attribute helper."""
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {CONF_REFERENCE_ENTITY_ID: ["climate.living_room"]}
    )
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        {CONF_SENSOR_CONVERSION: "climate.living_room:current_temperature"},
    )
    assert result["step_id"] == "entity_helper"
    result = await hass.config_entries.options.async_configure(
...
async def test_conversion_error_preserves_individual_measurements(hass):
    ...
    assert suggested_form_values(result["data_schema"]) == submitted
...
async def test_multiple_light_sources_create_live_average_brightness_sensor(hass):
    ...
    hass.states.async_set("light.second", "on", {"brightness": 0})
    entry = MockConfigEntry(
        domain=COMPONENT_DOMAIN,
        data={ATTR_GROUP_NAME: "ui"},
        options={ATTR_DEVICES: {}},
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {CONF_REFERENCE_ENTITY_ID: ["light.first", "light.second"]}
    )
    assert result["step_id"] == "entity_type"
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {CONF_TARGET_ENTITY_TYPE: "sensor"}
    ...
    assert result["step_id"] == "sensor_conversion"
    assert suggested_form_values(result["data_schema"])[CONF_SENSOR_AGGREGATION] == "average"
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        {
            CONF_SENSOR_CONVERSION: "brightness",
            CONF_SENSOR_AGGREGATION: "average",
        },
    )
    result = await hass.config_entries.options.async_configure(
    ...
    assert float(sensor.native_value) == pytest.approx(100.0)
    hass.states.async_set("light.second", "on", {"brightness": 255})
    sensor._apply_templates()
    assert sensor.available is True
    assert float(sensor.native_value) == pytest.approx(100.0)


async def test_multiple_pm25_sensors_normalize_units_in_add_flow(hass):
    """Sensor sources must enter conversion directly and aggregate one unit."""
    hass.states.async_set(
        "sensor.pm25_indoor",
        "10",
    ...
    assert float(sensor.native_value) == pytest.approx(15)
    assert sensor.device_class == "pm25"
    assert sensor.state_class == "measurement"
...
async def test_edit_flow_converts_climate_entity_to_live_temperature_sensor(hass):
    ...
    hass.states.async_set(
        "climate.source",
        "heat",
        {"current_temperature": 19.5, "temperature_unit": "°C"},
    )
    entry = MockConfigEntry(
        domain=COMPONENT_DOMAIN,
        data={ATTR_GROUP_NAME: "ui"},
                ...
                "Room": [
                    {
                        ...
                        CONF_NAME: "Room Climate",
                        ATTR_ENTITY_ID: "climate.room_virtual",
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {CONF_ENTITY_KEY: _entity_key("Room", 0)}
    )
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {CONF_REFERENCE_ENTITY_ID: ["climate.source"]}
    )
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], {CONF_TARGET_ENTITY_TYPE: "sensor"}
    )
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        {CONF_SENSOR_CONVERSION: "climate.source:current_temperature"},
    )
    result = await hass.config_entries.options.async_configure(
    ...
    assert defaults[ATTR_ENTITY_ID] == "sensor.room_virtual"
    assert defaults[CONF_NATIVE_VALUE_TEMPLATES]["device_class"] == (
        "{{ 'temperature' }}"
    )
    assert (
        defaults[CONF_NATIVE_VALUE_TEMPLATES]["native_unit_of_measurement"]
        == "{{ '°C' }}"
    )
...
async def test_multiple_boolean_domains_create_live_binary_sensor(hass):
    ...
    entity = virtual_binary_sensor.VirtualBinarySensor(
...
async def test_options_flow_persists_native_templates_and_command_actions(hass):
    ...
    assert {
        name: entity[CONF_NATIVE_TEMPLATES][name] for name in native_templates
    } == native_templates
...
async def test_options_flow_converts_single_switch_source_to_fan(hass):
    ...
    assert {option["value"] for option in target_selector.config["options"]} == {
        "switch",
        "fan",
        "light",
        "binary_sensor",
    ...
    assert defaults[CONF_NATIVE_VALUE_TEMPLATES]["is_on"] == (
        "{{ states('switch.desk_fan_power') not in ['off', 'unknown', 'unavailable'] }}"
    )
...
async def test_options_flow_edit_changes_single_switch_backed_entity_to_fan(hass):
    ...
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        _flatten_entity_form_sections(suggested_form_values(result["data_schema"])),
    )
    assert result["step_id"] == "edit_entity_helper"
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
    ...
    changed_defaults = _flatten_entity_form_sections(suggested_form_values(result["data_schema"]))
...
async def test_edit_type_step_preserves_legacy_custom_target_domain(hass):
    ...
    assert result["step_id"] == "edit_entity_type"
    assert suggested_form_values(result["data_schema"])[CONF_TARGET_ENTITY_TYPE] == "sensor"
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
    ...
    assert defaults[CONF_VALUE_TEMPLATE] == ("{{ states('switch.legacy_source') }}")
...
async def test_edit_automatic_repairs_legacy_switch_backed_fan_actions(hass):
    ...
    assert result["step_id"] == "edit_entity_type"
    assert suggested_form_values(result["data_schema"])[CONF_TARGET_ENTITY_TYPE] == "fan"
    result = await hass.config_entries.options.async_configure(
        result["flow_id"],
        {CONF_TARGET_ENTITY_TYPE: "fan"},
    )
...
async def test_options_flow_ignores_restored_source_metadata(hass):
    ...
    assert ATTR_RESTORED not in _yaml_value(defaults[CONF_ATTRIBUTE_TEMPLATES_JSON])
...
async def test_options_flow_camera_alias_tracks_native_camera_states(hass):
    ...
    assert set(camera._virtual_attributes).isdisjoint(TRANSIENT_SOURCE_ATTRIBUTE_NAMES)
    assert set(camera._attribute_templates).isdisjoint(TRANSIENT_SOURCE_ATTRIBUTE_NAMES)
...
async def test_options_flow_builds_and_runs_climate_hot_water_boiler_helper(hass):
    ...
    assert (
        "switch.hot_water"
        not in defaults[CONF_NATIVE_VALUE_TEMPLATES]["current_temperature"]
    )
...
async def test_boiler_air_conditioner_helper_routes_runtime_commands_and_values(hass):
    ...
    assert (
        Template(defaults[CONF_VALUE_TEMPLATE], hass).async_render(parse_result=True)
        == "off"
    )
...
async def test_combined_climate_restores_until_startup_sources_are_ready(hass):
    ...
    climate.hass = hass
    climate.async_get_last_state = AsyncMock(
        return_value=State(
            climate.entity_id,
            "heat_cool",
            {
...
async def test_options_flow_persists_every_combined_climate_helper_template(hass):
    ...
    assert (
        _yaml_value(defaults[CONF_COMMAND_ACTIONS_JSON])["set_temperature"][
            "optimistic"
        ]
        is False
    )
...
async def test_options_flow_rejects_invalid_jinja_before_saving(hass, caplog):
    ...
    assert result["type"] == FlowResultType.FORM
    assert result["errors"] == {CONF_ATTRIBUTE_TEMPLATES_JSON: "invalid_template"}
...
async def test_vacuum_edit_hides_json_and_preserves_native_templates(hass):
    ...
    defaults = _flatten_entity_form_sections(suggested_form_values(result["data_schema"]))
    assert (
        defaults[CONF_NATIVE_VALUE_TEMPLATES][managed_property]
...
async def test_climate_edit_repairs_legacy_enum_repr_native_template(hass):
    ...
    defaults = _flatten_entity_form_sections(suggested_form_values(result["data_schema"]))
    assert defaults[CONF_NATIVE_VALUE_TEMPLATES]["hvac_action"] == ("{{ 'heating' }}")
...
async def test_device_tracker_edit_repairs_legacy_enum_repr_attribute_template(hass):
    ...
    assert (
        saved[CONF_EVENT_HOOKS][0][CONF_ATTRIBUTE_TEMPLATES]["source_type"]
        == "{{ 'gps' }}"
    )
...
async def test_home_assistant_loads_korean_config_translations(hass):
    ...
    assert (
        config_translations["component.virtual_layer.config.step.entity.title"]
        == "가상 엔티티 추가"
    )
    assert (
        config_translations[
    ...
    assert (
        "Jinja 템플릿"
        in config_translations[
            "component.virtual_layer.config.step.entity.data_description.value_template"
        ]
    )
    assert (
        config_translations["component.virtual_layer.config.step.entity_helper.title"]
        == "템플릿 자동 생성 선택"
    )
    assert (
        "원본 엔티티"
    ...
    assert (
        config_translations[
            "component.virtual_layer.config.step.entity_helper.data.use_template_helper"
        ]
        == "원본 기반 템플릿 자동 생성"
    )
    assert (
    ...
    assert (
        options_translations["component.virtual_layer.options.step.entity_helper.title"]
        == "템플릿 자동 생성 선택"
    )
    assert (
        "원본 엔티티"
        in options_translations[
...
async def test_initial_config_flow_device_name_is_blank(hass):
    ...
    assert result["type"] == FlowResultType.FORM
    assert suggested_form_values(result["data_schema"])[ATTR_GROUP_NAME] == ""
...
async def test_config_entry_setup_recovers_and_normalizes_device_name(
    ...
    assert (
        hass.data[COMPONENT_DOMAIN][group_name][ATTR_CONFIG_ENTRY_ID] == entry.entry_id
    )
    forward_setups.assert_not_awaited()
...
async def test_options_flow_manages_shared_device_metadata_without_editing_entities(
    hass,
):
    entry = MockConfigEntry(
        domain=COMPONENT_DOMAIN,
...
async def test_options_flow_can_edit_existing_entity(hass):
    ...
    source_defaults = _flatten_entity_form_sections(suggested_form_values(result["data_schema"]))
...
async def test_options_flow_refreshes_generated_helper_when_sources_change(
    ...
    stored_auto_helper = (
        legacy_profile if auto_helper_marker == "legacy_profile" else auto_helper_marker
    )
    entry = MockConfigEntry(
        domain=COMPONENT_DOMAIN,
        data={ATTR_GROUP_NAME: "ui"},
                ...
                "Doors": [
                    {
                        ...
                        CONF_TEMPLATE_SOURCES: {
                            ...
                            for variable_name, source_entity_id in old_template_sources.items()
                        },
...
async def test_options_flow_recovers_stale_helper_after_partial_source_update(
    ...
    assert {
        source[ATTR_ENTITY_ID] for source in entity[CONF_TEMPLATE_SOURCES].values()
    } == set(new_sources)
...
async def test_options_flow_handles_custom_template_when_sources_change(
    ...
    custom_template = (
        generated[CONF_VALUE_TEMPLATE][:-3] + " and (door_5 != 'unavailable') }}"
    )
    entry = MockConfigEntry(
        domain=COMPONENT_DOMAIN,
        data={ATTR_GROUP_NAME: "ui"},
                ...
                "Doors": [
                    {
                        ...
                        CONF_TEMPLATE_SOURCES: {
                            ...
                            for variable_name, source_entity_id in template_sources.items()
                        },
...
async def test_edit_form_source_change_requires_helper_policy(hass):
    ...
    result = await _accept_binary_detection_defaults(hass, result)
    edited = _flatten_entity_form_sections(suggested_form_values(result["data_schema"]))
    ...
    assert (
        entity[CONF_AUTO_HELPER][CONF_VALUE_TEMPLATE] == generated[CONF_VALUE_TEMPLATE]
    )
...
async def test_repeated_source_changes_regenerate_from_latest_helper_baseline(hass):
    ...
    result = await _accept_binary_detection_defaults(hass, result)
    final_defaults = _flatten_entity_form_sections(suggested_form_values(result["data_schema"]))
...
async def test_options_flow_refreshes_untouched_native_jinja_and_keeps_custom_field(
    ...
    assert (
        native_templates["hvac_mode"]
        == expected[CONF_NATIVE_VALUE_TEMPLATES]["hvac_mode"]
    )
    assert (
        native_templates["hvac_modes"]
        == expected[CONF_NATIVE_VALUE_TEMPLATES]["hvac_modes"]
    )
    ...
    assert actual_entities == [
        {
            ...
            CONF_AVAILABILITY_TEMPLATE: (
                "{{ states('light.kitchen_lamp') not in ['unknown', 'unavailable'] }}"
            ),
            CONF_ATTRIBUTES: {"brightness": 128},
        },
    ]
    assert set(native_templates) == set(DOMAIN_NATIVE_TEMPLATE_PROPERTIES["light"])
    assert native_templates["is_on"]
    assert native_templates["brightness"]
    assert native_templates["is_on"] == (
        "{{ states('light.kitchen_lamp') not in ['off', 'unknown', 'unavailable'] }}"
    )
    assert (
        Template(native_templates["brightness"], hass).async_render(parse_result=True)
        == 128
    )
...
async def test_creation_flows_apply_the_selected_template_helper_policy(hass):
    ...
    assert result["step_id"] == "entity"
    config_defaults = _flatten_entity_form_sections(suggested_form_values(result["data_schema"]))
...
@pytest.mark.parametrize("submitted_id", ["sensor.saved_id", "", "   ", None])
@pytest.mark.parametrize("name", ["Original", "Powder Focused Light"])
async def test_edit_name_preserves_id_unless_cleared(hass, submitted_id, name):
    ...
    if submitted_id is None:
        # Clearing optional text in HA's frontend removes the key entirely.
        submitted.pop(ATTR_ENTITY_ID)
    submitted = result["data_schema"](submitted)
    result = await manager.async_configure(result["flow_id"], submitted)
    assert result["type"] == FlowResultType.CREATE_ENTRY
    expected = "sensor.original" if name == "Original" else "sensor.pd_fclight"
    assert _first_stored_entity(result)[ATTR_ENTITY_ID] == (
        (submitted_id or "").strip() or expected
    )
...
async def test_options_flow_refreshes_untouched_helpers_when_add_sources_change(hass):
    ...
    assert {
        source[ATTR_ENTITY_ID] for source in saved[CONF_TEMPLATE_SOURCES].values()
    } == set(new_sources)
...
async def test_helper_update_failure_keeps_edit_form_available(
...
async def test_options_flow_prefills_climate_native_mode_options(hass):
    ...
    assert (
        Template(native_templates["fan_mode"], hass).async_render(parse_result=True)
        == "auto"
    )
    assert (
        Template(native_templates["target_temperature"], hass).async_render(
            parse_result=True
        )
        == 23.0
    )
    ...
    assert ClimateEntityFeature.PRESET_MODE in climate.supported_features
    assert (
        ClimateEntityFeature.TARGET_TEMPERATURE_RANGE not in climate.supported_features
    )
...
async def test_options_flow_combines_fans_and_routes_matter_speed_to_stepped_source(
    hass,
):
    ...
    role_input = suggested_form_values(result["data_schema"])
...
async def test_options_flow_edits_stepped_fan_with_matter_levels_in_place(hass):
    ...
    assert result["step_id"] == "edit_entity"
    result = await hass.config_entries.options.async_configure(
        result["flow_id"], _flatten_entity_form_sections(suggested_form_values(result["data_schema"]))
    )
...
async def test_options_flow_combines_xiaomi_fan_and_speed_number(hass):
    ...
    assert (
        Template(entity[CONF_AVAILABILITY_TEMPLATE], hass).async_render(
            parse_result=True
        )
        is True
    )
...
async def test_options_flow_prefills_and_creates_native_dehumidifier(hass):
    ...
    native_templates = defaults[CONF_NATIVE_VALUE_TEMPLATES]
    assert (
        Template(native_templates["device_class"], hass).async_render(parse_result=True)
        == "dehumidifier"
    )
    ...
    assert (
        Template(native_templates["target_humidity"], hass).async_render(
            parse_result=True
        )
        == 50
    )
...
async def test_options_flow_adds_entity_to_selected_existing_device(hass):
    ...
    assert result["data"][ATTR_DEVICE_ATTRIBUTES]["Refrigerator Door"][
        ATTR_DEVICE_ID
    ] == ("refrigerator-door-1")


async def test_setup_entry_groups_multiple_virtual_entities_on_one_device(
...
async def test_setup_entry_restores_stale_virtual_entity_registry_metadata(
    ...
    info = entity_registry.async_get("sensor.src_refrigerator_door_info")
...
async def test_presence_motion_helper_retains_detected_state_until_all_sources_clear(
    hass,
):
    source_ids = [
    ...
    assert (
        hass.states.get("binary_sensor.combined_motion").attributes["device_class"]
        == "motion"
    )
...
async def test_setup_entries_reserve_unique_ids_for_matching_generated_entities(hass):
    ...
    entity_registry = er.async_get(hass)
    assert (
        entity_registry.async_get(first_entity[ATTR_ENTITY_ID]).config_entry_id
        == first_entry.entry_id
    )
    assert (
        entity_registry.async_get(second_entity[ATTR_ENTITY_ID]).config_entry_id
...
async def test_setup_entry_repairs_unique_id_owned_by_another_entry(
    ...
    assert (
        registry.async_get(first_entity[ATTR_ENTITY_ID]).config_entry_id
        == first_entry.entry_id
    )
    assert (
        registry.async_get(second_entity[ATTR_ENTITY_ID]).config_entry_id
        == second_entry.entry_id
    )
...
async def test_diagnostic_registry_defaults_are_migrated_without_overwriting_user_customization(
            ...
            "sensor": [
                {
                    ...
                    CONF_NAME: "[Source] - Virtual Entity - Configuration",
                    CONF_ICON: "mdi:information-outline",
                    CONF_ATTRIBUTES: {"diagnostic_type": "configuration"},
                }
            ],
        },
    )
    ...
    assert migrated.original_name == "[Source] - Virtual Entity - Configuration"
...
async def test_shared_device_metadata_guards_converge_without_update_loop(hass):
    ...
    assert (
        registry.async_get(device.id).manufacturer
        == configured_manufacturers[owner_entry_id]
    )


async def test_setup_entry_removes_orphaned_entity_and_device_registry_entries(
    hass, tmp_path, monkeypatch
):
    meta_file = tmp_path / "virtual_layer.meta.json"
    meta_file.write_text(
...
async def test_setup_entry_removes_stale_device_after_device_id_change(
    hass, tmp_path, monkeypatch
):
    meta_file = tmp_path / "virtual_layer.meta.json"
    monkeypatch.setattr(
...
async def test_setup_entry_syncs_metadata_and_allows_clearing_device_area(
    ...
    kitchen = ar.async_get(hass).async_create("Kitchen")
    entry.options[ATTR_DEVICE_ATTRIBUTES]["Child"][CONF_VIA_DEVICE_ID] = (
        parent_device.id
    )
    ...
    assert CONF_SUGGESTED_AREA not in entry.options[ATTR_DEVICE_ATTRIBUTES]["Child"]
...
async def test_setup_entry_clears_removed_entity_default_icon(
    hass, tmp_path, monkeypatch
):
    meta_file = tmp_path / "virtual_layer.meta.json"
    monkeypatch.setattr(
    ...
    await hass.async_block_till_done()
    assert (
        er.async_get(hass).async_get("sensor.virtual_meter").original_icon
        == "mdi:flash"
    )
...
async def test_device_id_change_moves_existing_entity_to_new_device(
    hass, tmp_path, monkeypatch
):
    meta_file = tmp_path / "virtual_layer.meta.json"
    monkeypatch.setattr(
...
async def test_state_only_reload_removes_attributes_deleted_from_configuration(
    ...
    assert set(restored.attributes).isdisjoint(TRANSIENT_SOURCE_ATTRIBUTE_NAMES)
...
def test_climate_schema_accepts_template_sources_and_default_state():
    ...
    assert (
        config[CONF_TEMPLATE_SOURCES]["outdoor_temperature"][ATTR_ENTITY_ID]
        == "sensor.outdoor_temperature"
    )
...
async def test_custom_event_bus_hook_filters_and_updates_virtual_entity(hass):
    ...
    hass.bus.async_fire(
        "virtual_layer_manual_update", {"target": "dryer", "value": "drying"}
    )
    await hass.async_block_till_done()
    assert entity._attr_state == "idle"

    hass.bus.async_fire(
        "virtual_layer_manual_update", {"target": "washer", "value": "rinse"}
    )
    await hass.async_block_till_done()

    assert entity._attr_state == "rinse"
    assert (
        entity.extra_state_attributes["source_event"] == "virtual_layer_manual_update"
    )
    await entity.async_will_remove_from_hass()
...
async def test_state_only_entity_registry_preserves_configured_icon(
    hass, tmp_path, monkeypatch
):
    meta_file = tmp_path / "virtual_layer.meta.json"
    monkeypatch.setattr(
...
async def test_state_only_entity_updates_composite_templates(
    hass, tmp_path, monkeypatch
):
    meta_file = tmp_path / "virtual_layer.meta.json"
    monkeypatch.setattr(
...
async def test_state_only_setup_does_not_overwrite_existing_state(
    hass, tmp_path, monkeypatch
):
    meta_file = tmp_path / "virtual_layer.meta.json"
    monkeypatch.setattr(
...
async def test_device_tracker_service_is_registered_once(hass):
    ...
    await virtual_device_tracker.async_setup_entry(
        fake_hass, entry, lambda entities: None
    )
    await virtual_device_tracker.async_setup_entry(
        fake_hass, entry, lambda entities: None
    )
...
async def test_startup_completion_rechecks_missed_source_availability(hass):
    ...
    availability_template = (
        "{{ states('sensor.late_startup_source') not in ['unknown', 'unavailable'] }}"
    )
    regular_config = {
    ...
    with (
        ...
        assert (
            hass.states.get(state_only[ATTR_ENTITY_ID]).attributes[ATTR_AVAILABLE]
            is False
        )
    ...
    assert (
        hass.states.get(state_only[ATTR_ENTITY_ID]).attributes[ATTR_AVAILABLE] is True
    )
...
async def test_generated_attribute_helpers_follow_multiple_source_attributes(hass):
    ...
    config = {
        ...
        CONF_INITIAL_AVAILABILITY: True,
        CONF_PERSISTENT: False,
        CONF_ATTRIBUTE_TEMPLATES: _yaml_value(defaults[CONF_ATTRIBUTE_TEMPLATES_JSON]),
    }
    entity = VirtualSensor(config, False)
    entity.hass = hass
    entity.async_schedule_update_ha_state = Mock()
    ...
    with patch.object(hass.config_entries, "async_reload", wraps=hass.config_entries.async_reload) as reload:
        ...
        for records in ([changed], [edited], [], [changed], []):
            ...
            if records:
                ...
                assert hass.states.get("sensor.src_changed_info") is None
...
@pytest.mark.parametrize("source_value,source_class,source_unit,expected", [
    ("150", "aqi", "AQI", "moderate"),
    ("54.07", "radon", "Bq/m³", "fair"),
    ("40", "pm25", "μg/m³", "moderate"),
    ("15", "pm1", "μg/m³", "moderate"),
    ("0.05", "ozone", "ppm", "moderate"),
...
async def test_air_quality_default_skips_rules_and_preserves_source(hass, tmp_path, monkeypatch, source_value, source_class, source_unit, expected, per_source):
    ...
    source_id = f"sensor.original_{source_class}"
    source_attributes = {"device_class": source_class, "unit_of_measurement": source_unit}
    if source_class in {"benzene", "ammonia", "hydrogen_sulfide", "etvoc"}:
        # These inputs have names/units, not invented HA device classes.
        source_attributes.pop("device_class")
        source_attributes["friendly_name"] = source_class
    hass.states.async_set(source_id, source_value, source_attributes)
    original = hass.states.get(source_id)
    hass.states.async_set("sensor.second_pm1", "1", {"device_class": "pm1", "unit_of_measurement": "μg/m³"})
    source_ids = [source_id, "sensor.second_pm1"] if per_source else [source_id]
    entry = MockConfigEntry(domain=COMPONENT_DOMAIN, data={ATTR_GROUP_NAME: "auto_air"}, options={ATTR_DEVICES: {}})
    entry.add_to_hass(hass)
    result = await hass.config_entries.options.async_init(entry.entry_id, data={CONF_ACTION: ACTION_ADD_ENTITY})
    result = await hass.config_entries.options.async_configure(result["flow_id"], {CONF_REFERENCE_ENTITY_ID: source_ids})
    if result["step_id"] == "sensor_conversion":
        result = await hass.config_entries.options.async_configure(result["flow_id"], suggested_form_values(result["data_schema"]))
    ...
    if values[CONF_PLATFORM] != "air_quality":
        values[CONF_PLATFORM] = "air_quality"
    ...
    values["air_quality_per_source"] = per_source
    values[ATTR_ENTITY_ID] = "air_quality.automatic_test"
    assert Template(values[CONF_NATIVE_VALUE_TEMPLATES]["air_quality"], hass).async_render() == expected
    result = await hass.config_entries.options.async_configure(result["flow_id"], values)
    assert result["type"] == FlowResultType.CREATE_ENTRY
    ...
    await asyncio.sleep(0.1)
    await hass.async_block_till_done()
    assert hass.states.get("air_quality.automatic_test").state == expected
    ...
    child = hass.states.get(f"sensor.automatic_test_{source_id.replace('.', '_')}_air_quality")
    if per_source:
        assert child.state == expected
        assert child.attributes["source_entity_id"] == source_id
        assert "unit_of_measurement" not in child.attributes
        registry = er.async_get(hass)
        assert registry.async_get(child.entity_id).device_id == registry.async_get(bridge.entity_id).device_id
        second_id = "sensor.automatic_test_sensor_second_pm1_air_quality"
        assert hass.states.get(second_id).state == "good"
        # Disabling the option must clean up both generated sensors, not sources.
        options = deepcopy(dict(entry.options))
        for records in options[ATTR_DEVICES].values():
            for record in records:
                if record.get(ATTR_ENTITY_ID) == "air_quality.automatic_test":
                    record[CONF_AIR_QUALITY_LOGIC]["per_source"] = False
        hass.config_entries.async_update_entry(entry, options=options)
        await hass.async_block_till_done()
        assert hass.states.get(child.entity_id) is None
        assert hass.states.get(second_id) is None
        assert registry.async_get(child.entity_id) is None
        assert hass.states.get("sensor.second_pm1").state == "1"
    else:
        assert child is None
    assert hass.states.get(source_id) is original


@pytest.mark.parametrize("scope", [None, "combined", "sources", "leaves"])
async def test_new_measurement_flow_creates_aqi_without_air_quality_step(hass, tmp_path, monkeypatch, scope):
    monkeypatch.setattr("custom_components.virtual_layer.cfg.default_meta_file", lambda hass: str(tmp_path / "auto-sensor.meta.json"))
    hass.states.async_set("sensor.physical_pm25", "5", {"device_class": "pm25", "unit_of_measurement": "μg/m³"})
    original = hass.states.get("sensor.physical_pm25")
    entry = MockConfigEntry(domain=COMPONENT_DOMAIN, data={ATTR_GROUP_NAME: "air"}, options={ATTR_DEVICES: {}})
    entry.add_to_hass(hass)
    result = await hass.config_entries.options.async_init(entry.entry_id, data={CONF_ACTION: ACTION_ADD_ENTITY})
    result = await hass.config_entries.options.async_configure(result["flow_id"], {CONF_REFERENCE_ENTITY_ID: ["sensor.physical_pm25"]})
    result = await _choose_add_template_helper(hass, result, target_entity_type="sensor")
    assert result["step_id"] == "entity"
    values = _flatten_entity_form_sections(suggested_form_values(result["data_schema"]))
    values[ATTR_ENTITY_ID] = "sensor.ui_pm25"
    if scope:
        values["configure_air_quality_sources"] = True
        result = await hass.config_entries.options.async_configure(result["flow_id"], values)
        assert result["step_id"] == "air_quality_scope"
        result = await hass.config_entries.options.async_configure(result["flow_id"], {
            "scope": scope, "sources": ["sensor.physical_pm25"], "missing": "skip"})
        assert result["step_id"] == "air_quality_source_rules"
        source = "sensor.ui_pm25" if scope == "combined" else "sensor.physical_pm25"
        result = await hass.config_entries.options.async_configure(result["flow_id"], {"source": source, "action": "edit"})
        assert result["step_id"] == "air_quality_setup"
        values = suggested_form_values(result["data_schema"])
        values.update({f"boundary_{i+1}": value for i, value in enumerate([1, 2, 3, 4, 10])})
        result = await hass.config_entries.options.async_configure(result["flow_id"], values)
        assert result["step_id"] == "air_quality_source_rules"
        result = await hass.config_entries.options.async_configure(result["flow_id"], {"source": source, "action": "continue"})
        assert result["step_id"] == "entity"
        values = _flatten_entity_form_sections(suggested_form_values(result["data_schema"]))
    result = await hass.config_entries.options.async_configure(result["flow_id"], values)
    assert result["type"] == FlowResultType.CREATE_ENTRY
    ...
    if scope:
        ...
        record = next(iter(entry.options[ATTR_DEVICES].values()))[0]
        assert record["air_quality_logic"]["measurements"][0]["thresholds"] == [1, 2, 3, 4, 10]
...
async def test_direct_air_quality_source_profile_persists_and_drives_companion(hass, tmp_path, monkeypatch, scope):
    monkeypatch.setattr("custom_components.virtual_layer.cfg.default_meta_file", lambda hass: str(tmp_path / "source-rules.meta.json"))
    hass.states.async_set("sensor.physical_pm25", "15", {"device_class": "pm25", "unit_of_measurement": "μg/m³"})
    original = hass.states.get("sensor.physical_pm25")
    options = {ATTR_DEVICES: {}}
    reference_id = "sensor.physical_pm25"
    if scope == "leaves":
        reference_id = "sensor.combined_pm25"
        hass.states.async_set(reference_id, "15", {"device_class": "pm25", "unit_of_measurement": "μg/m³"})
        options = {ATTR_DEVICES: {"Existing": [{CONF_PLATFORM: "sensor", CONF_NAME: "Combined PM25",
            ATTR_ENTITY_ID: reference_id, CONF_SOURCE_ENTITIES: ["sensor.physical_pm25"],
            CONF_VALUE_TEMPLATE: "{{ states('sensor.physical_pm25') }}", "device_class": "pm25", "unit_of_measurement": "μg/m³"}]}}
    entry = MockConfigEntry(domain=COMPONENT_DOMAIN, data={ATTR_GROUP_NAME: "air"}, options=options)
    entry.add_to_hass(hass)
    result = await hass.config_entries.options.async_init(entry.entry_id, data={CONF_ACTION: ACTION_ADD_ENTITY})
    result = await hass.config_entries.options.async_configure(result["flow_id"], {CONF_REFERENCE_ENTITY_ID: [reference_id]})
    result = await _choose_add_template_helper(hass, result, target_entity_type="air_quality")
    values = _flatten_entity_form_sections(suggested_form_values(result["data_schema"]))
    values.update({ATTR_ENTITY_ID: "air_quality.ui_profile", "configure_air_quality_sources": True, "air_quality_per_source": True})
    result = await hass.config_entries.options.async_configure(result["flow_id"], values)
    assert result["step_id"] == "air_quality_scope"
    result = await hass.config_entries.options.async_configure(result["flow_id"], {
        "scope": scope, "sources": [reference_id], "missing": "skip"})
    result = await hass.config_entries.options.async_configure(result["flow_id"], {"source": "sensor.physical_pm25", "action": "edit"})
    values = suggested_form_values(result["data_schema"])
    values.update({f"boundary_{i+1}": value for i, value in enumerate([20, 30, 40, 50, 60])})
    result = await hass.config_entries.options.async_configure(result["flow_id"], values)
    result = await hass.config_entries.options.async_configure(result["flow_id"], {"source": "sensor.physical_pm25", "action": "continue"})
    assert result["step_id"] == "entity"
    values = _flatten_entity_form_sections(suggested_form_values(result["data_schema"]))
    result = await hass.config_entries.options.async_configure(result["flow_id"], values)
    assert result["type"] == FlowResultType.CREATE_ENTRY
...
async def test_air_quality_uses_a_dedicated_matter_configuration_step(hass):
    ...
    assert result["type"] == FlowResultType.FORM
    assert result["step_id"] == "entity"
    manual = _flatten_entity_form_sections(suggested_form_values(result["data_schema"]))
    ...
    assert suggested_form_values(result["data_schema"])["mode"] == "fixed"
    result = await hass.config_entries.options.async_configure(
...
async def test_add_formaldehyde_air_quality_preserves_same_named_sensor(hass, tmp_path, monkeypatch, via_edit):
    ...
    result = await hass.config_entries.options.async_configure(result["flow_id"], manual)
    assert result["step_id"] == "air_quality"
    assert suggested_form_values(result["data_schema"])["mode"] == "automatic"
...
async def test_air_quality_measurement_recipe_precedes_templates_and_validates(hass):
    ...
    assert result["step_id"] == "air_quality_setup"
    assert suggested_form_values(result["data_schema"])["boundary_5"] == 50
    result = await hass.config_entries.options.async_configure(result["flow_id"], values)
    ...
    limits = suggested_form_values(result["data_schema"])
    ...
    result = await hass.config_entries.options.async_configure(result["flow_id"], {"next_action": "continue"})
    assert result["step_id"] == "edit_entity"
    updated = _flatten_entity_form_sections(suggested_form_values(result["data_schema"]))
